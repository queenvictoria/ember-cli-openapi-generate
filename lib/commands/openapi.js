...
module.exports = {
  ...
  availableOptions: [
    ...
    { name: 'resource', type: Boolean, description: 'Generate resources instead of models (models, routes, templates).' },
    ...
    if ( name !== 'error' ) {
      ...
      if (
        ...
        _.each(attributes, function(attribute, attrName) {
          ...
          attrBlueprint[attrName] = attribute.type;
        ...
        self.ui.writeLine(`ember generate model ${modelName} ${attrCli.join(' ')}`);

        // @TODO Write out a model file.
        // GenerateTask
        // Model or Resource
        let blueprintName = "model";
        // let blueprintName = "resource";

        let options = {
          entity: {
            name: modelName, 
            options: attrBlueprint,
          },
          target: "./",
          ui: self.ui,
